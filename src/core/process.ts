...
function deEmphasisUnchecked(samples: Float32Array, coefficient: number = 0.97): Float32Array {
  if (!Number.isFinite(coefficient) || coefficient < 0 || coefficient >= 1) {
...
export function deEmphasis(samples: Float32Array, coefficient: number = 0.97): Float32Array {
  return processSafely(samples, (copy) => deEmphasisUnchecked(copy, coefficient));
}
