...
describe("padding", () => {
  ...
  it("ensureLength truncates when input is longer than target", () => {
    const out = ensureLength([1, 2, 3, 4, 5], 3);
    expect(out.length).toBe(3);
    expect(Array.from(out)).toEqual([1, 2, 3]);
  });

  it("ensureLength returns a copy when input matches target exactly", () => {
    const input = new Float32Array([10, 20, 30]);
    const out = ensureLength(input, 3);
    expect(out.length).toBe(3);
    expect(Array.from(out)).toEqual([10, 20, 30]);
    expect(out).not.toBe(input);
  });

  it("padConstant uses 0 as the default value", () => {
    const out = padConstant([1, 2], 1, 1);
    expect(Array.from(out)).toEqual([0, 1, 2, 0]);
  });

  it("padEdge on empty input returns all zeros", () => {
    const out = padEdge([], 2, 2);
